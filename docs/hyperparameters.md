...
| **distill** `extra` | true | principled | The MobileNet result showed the gap is the training regime, not the data ceiling; the 60k unused images are labelled and free. Makes train unbalanced (62.5k/32.5k) — soft targets dominate so this is usually fine; `false` gives the clean soft-vs-hard comparison at 35k. | Both to be run |
