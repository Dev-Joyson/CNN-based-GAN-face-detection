...
| `stem_stride` | 1 | ~ cited, **test pending** | Gragnaniello et al. 2021: early downsampling destroys the GAN fingerprint. Stride 1 keeps the first block at full 256² — the source of the model's 1.3 GB peak memory and 1.11 G MACs (MobileNet: 0.07 G). Never tested on this data. | **test21_stride2_stem**: stride 2 → 0.28 G MACs, same params. Accuracy holds → adopt it; drops → the citation is verified here |
